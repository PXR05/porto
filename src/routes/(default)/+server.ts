import { contacts, flatSkills, profile, projects } from '@/lib/data';
...
function formatForTerminal(): string {
	...
	Object.values(contacts).forEach((contact) => {
		const paddedKey = (contact.label + ':').padEnd(15);
		output += `${paddedKey} ${contact.value}\n`;
	});
	output += '\n';
	...
	for (let i = 0; i < flatSkills.length; i += skillsPerRow) {
		const row = flatSkills.slice(i, i + skillsPerRow);
		output += `${row.join(' • ')}\n`;
	}
	output += '\n';
