			...
			{#each Object.entries(contacts).map( ([key, value]) => ({ label: key, link: key === 'Email' ? `mailto:${value}` : value, content: value
				...
				>
					{#if inview}
						...
						>
							<span
								class="absolute -right-2 -top-2 aspect-square h-4 w-4 rounded-full bg-background"
							></span>
							<span
						...
						<h2
							...
							class="flex w-full justify-between border-b-2 border-transparent font-serif text-2xl transition-colors group-hover:border-primary max-md:flex-col md:items-center md:text-4xl"
						>
							<span>
								{label}
							</span>
